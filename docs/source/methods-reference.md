...
- **Post Keepawake Behavior**:  [Resetting](#idle-timer-resetting) type on KDE (KDE Plasma 5.27.11, tested by [reidswanson](https://github.com/reidswanson) on [#366](https://github.com/fohrloop/wakepy/discussions/366#discussioncomment-9797027)).
...
- **Post Keepawake Behavior**:  [Resetting](#idle-timer-resetting) type on GNOME 47 ([#471](https://github.com/fohrloop/wakepy/issues/471)). The behavior is likely to depend on the used DE.
...
- **Post Keepawake Behavior**:  [Resetting](#idle-timer-resetting) type (GNOME 47, [#471](https://github.com/fohrloop/wakepy/issues/471)).
...
- **Post Keepawake Behavior**:  **Up to Windows 10**: [Non-Resetting](#idle-timer-non-resetting) with about 2 minutes of [Post-Exit Inhibit](#post-exit-inhibit). The post-exit inhibit time seems to be a bit random, but it's very likely that any post-exit inhibit duration is between 1:30min and 3:30min (See: [#450](https://github.com/fohrloop/wakepy/issues/450#issuecomment-3074518760)). **Starting from Windows 11**: Just [Non-Resetting](#idle-timer-non-resetting), without post-exit inhibit. The post-exit inhibit behavior is also "documented" on [this question](https://learn.microsoft.com/en-us/answers/questions/999348/setthreadexecutionstate-without-es-continuous-does) on learn.microsoft.com (Sep 8, 2022), where the answer said: *"Prior to Windows 11 power requests were held an additional 2 minutes after they were dropped by the application. [...] This behavior was removed in Windows 11 and now systems are eligible to sleep as soon as the last power request is dropped"*.
