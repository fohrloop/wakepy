...
```{note}
When using the `with` statement, the context manager takes care of calling `Mode._deactivate()` if the `USER_CORE` raises an Exception. It is also possible to use `Mode._deactivate()` directly, but that is not recommended, as explained in [this Note](activating-a-mode-note) in the Activating a Mode section.
```
