...
try:
    ...
    raise NotImplementedError(
        "wakepy has not yet support for init processes other than systemd. "
...
def set_keepawake(keep_screen_awake=False):
    ...
    try:
        ...
        # print(power_manager.HasInhibit()) # Prints out if the inhibitor is
        # set ot not.
    except Exception as e:
        print(
            f"DBus Inhibit failed with a {type(e).__name__}: {e}\nFalling back to "
            "systemctl mask..."
        )
        subprocess.run([COMMAND, "mask", *ARGS])
...
def unset_keepawake():
    try:
        ...
        print(
            f"DBus UnInhibit failed with a {type(e).__name__}: {e}\nFalling back to "
            "systemctl mask..."
        )
        subprocess.run([COMMAND, "unmask", *ARGS])
